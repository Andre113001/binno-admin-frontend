...
import { Button, RadioGroup, FormControlLabel, Radio, CircularProgress, Skeleton } from '@mui/material';
...
const Reports = () => {
    ...
    const handleChangeRange = async (event) => {
        ...
        const blog_dates = blogRes.map(item => moment(item.stat_date).format('MM/DD'));
        const post_dates = postRes.map(item => moment(item.stat_date).format('MM/DD'));
        const event_dates = eventRes.map(item => moment(item.stat_date).format('MM/DD'));
        const guide_dates = guideRes.map(item => moment(item.stat_date).format('MM/DD'));
        ...
        const subscribeWeek = subscribeRes.map(item => moment(item.stat_date).format('MM/DD'));
        ...
        <Fragment>
            ...
            <main className='container'>
                ...
                <div ref={contentToPrint} className={styles["printable"]}>
                        ...
                        <div className={styles['date']}>
                            <section>
                                ...
                                {selectedRange === 'This Week' && (
                                    <>
                                        <h1 className={styles['day']}><Moment format='dddd'>{currentDateTime}</Moment></h1>
                                        <h1 className={styles['full-date']}><Moment format='MMMM DD, YYYY'>{currentDateTime}</Moment></h1>
                                    </>
                                )} 
                                {selectedRange === 'This Month' && (
                                    <>
                                        <h1 className={styles['day']}><Moment format='MMMM'>{currentDateTime}</Moment></h1>
                                        <h1 className={styles['full-date']}><Moment format='YYYY'>{currentDateTime}</Moment></h1>
                                    </>
                                )}
                                {selectedRange === 'This Year' && (
                                    <>
                                        <h1 className={styles['day']} style={{fontSize: 100, marginBottom: 60}}><Moment format='YYYY'>{currentDateTime}</Moment></h1>
                                    </>
                                )}
                            </section>
                        </div>
                    ...
                    {isLoading ? (
                        <Fragment>
                            <Skeleton variant="text" width={'20%'} height={60} /> {/* Adjust width and height as needed */}
                            <Skeleton variant="rectangular" width={'100%'} height={200} /> {/* Adjust width and height as needed */}
                        ...
                        <Fragment>
                            ...
                            <section className={styles['members-section']}>
                                <div className={`${styles['members-pie-chart']}`}>
                                    <PieChart
                                        ...
                                        {
                                            data: [
                                            { id: 0, value: fetchedData?.memberStat.count_enablers, label: 'Enabler', color: '#2c90c8' },
                                            { id: 1, value: fetchedData?.memberStat.count_company, label: 'Company', color: '#ff7a00' },
                                            ],
                                ...
                                <div className={styles['recently-joined']}>
                                        ...
                                        <div className={styles['list']}>
                                            {recentCompanies.length === 0 ? (
                                                ...
                                                recentCompanies.map((company, index) => (
                                                    <div className={styles['list-item']} key={index} onClick={() => {
                                                        ...
                                                        <div className={styles['list-details']}>
                                                            <p><b>{company.setting_institution}</b></p>
                                        ...
                                        {recentEnablers.length === 0 ? (
                                            ...
                                            recentEnablers.map((enabler, index) => (
                                                <div className={styles['list-item']} key={index} onClick={() => {
                                                    ...
                                                    <div className={styles['list-details']}>
                                                        <p><b>{enabler.setting_institution}</b></p>
                                    ...
                                    <div className={styles['more-btn']}>
                                        <Button sx={{w: 10}} fullWidth onClick={() => {
                                            navigate('/membership')
                                        }} >See More</Button>
                                    </div>
                                </div>
                            ...
                            <section className={styles['content-section']}>
                                ...
                                <div className={styles['content-metrics']}>
                                    <div className={styles['metrics']}>
                                        <h1 className={styles['metric-number']}>{isLoading ? (<CircularProgress />) : (fetchedData?.postStat.total === null ? 0 : fetchedData?.postStat.total  )}</h1>
                                        <h5 className={styles['metric-type']}>Posts</h5>
                                    </div>

                                    <div className={styles['metrics']}>
                                        <h1 className={styles['metric-number']}>{isLoading ? (<CircularProgress />) : (fetchedData?.blogStat.total === null ? 0 : fetchedData?.blogStat.total)}</h1>
                                        <h5 className={styles['metric-type']}>Blogs</h5>
                                    </div>

                                    <div className={styles['metrics']}>
                                        <h1 className={styles['metric-number']}>{isLoading ? (<CircularProgress />) : (fetchedData?.eventStat.total === null ? 0 : fetchedData?.eventStat.total)}</h1>
                                        <h5 className={styles['metric-type']}>Events</h5>
                                    </div>

                                    <div className={styles['metrics']}>
                                        <h1 className={styles['metric-number']}>{isLoading ? (<CircularProgress />) : (fetchedData?.guideStat.total === null ? 0 : fetchedData?.guideStat.total)}</h1>
                            ...
                            <h1 className={styles['section-title']}>Newsletter Subscribers</h1>
                            <section className={styles['newsletter-section']}>
                                <div className={styles["newsletter-metric"]}>
                                    <h1 className={styles['newsletter-metric-number']}>{isLoading ? (<CircularProgress />) : (fetchedData?.newsletterStat.total_subscriber === null || 0 ? 0 : fetchedData?.newsletterStat.total_subscriber)}</h1>
                                    <span className={styles['newsletter-metric-label']}>Total Subscribers</span>
                                </div>
